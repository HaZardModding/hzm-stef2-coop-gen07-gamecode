...
void CG_DrawText_UI_f()
{
	...
	//X
	std::string color = CTF::TrimColor(cgi.Argv(1)).c_str();
	float       x     = ParseCoordString(color, FONT_WIDTH * scale * CTF::TrimColor(msg.c_str()).length(), 0, SCREEN_W);
	
	//Y
	color   = CTF::TrimColor(cgi.Argv(2)).c_str();
	float y = ParseCoordString(color, FONT_HEIGHT * scale, 0, SCREEN_H);
