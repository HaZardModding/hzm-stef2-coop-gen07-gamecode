...
void ClientGameCommandManager::GetOrientation
   ...
   orientation = cgi.Tag_LerpedOrientation( current_tiki, current_entity, tagnum );

	VectorCopy( current_entity->origin, sp->cgd.origin );
	
   for ( i = 0 ; i < 3 ; i++ )
      {
		VectorMA( sp->cgd.origin, orientation.origin[i], current_entity->axis[i], sp->cgd.origin );
	   }
   
	MatrixMultiply( orientation.axis, current_entity->axis, sp->axis );
