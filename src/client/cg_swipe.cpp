...
void ClientGameCommandManager::Swipe
   ...
   for ( swipenum=0; swipenum < MAX_SWIPES; swipenum++ )
      ...
      if ( swipe.enabled && swipe.entitynum == current_entity_number )
         ...
         orientation = cgi.Tag_LerpedOrientation( current_tiki, current_entity, tagnum_start );
         ...
         for ( i = 0; i < 3; i++ )
            {
            VectorMA ( tmp, orientation.origin[i], current_entity->axis[i], tmp );
            }
         ...
         orientation = cgi.Tag_LerpedOrientation( current_tiki, current_entity, tagnum_end );
