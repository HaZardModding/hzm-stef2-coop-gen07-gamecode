...
void gamefix_replaceSubstring(str& source, const str& find, const str& replace)
{
	...
	for (int Q = 0; Q < srcLen; ){
		bool found = false;
		// Only try to match if there's enough remaining length
		if (Q + findLen <= srcLen){
			found = true;
			for (int Picard = 0; Picard < findLen; Picard++) {
				if (source[Q + Picard] != find[Picard]) {
					found = false;
					break;
				}
			}
...
{
	return gamefix_findString(string, find, true);
}
int gamefix_findString(const char* string, const char* find, bool caseSensitive)
{
	if (caseSensitive == true) {
		const char* pos = strstr(string, find);
		if (pos != nullptr) {
			return pos - string;
		}
	}
	else {
		str strTemp1Soure = string;
		str strTemp2Find = find;
		strTemp1Soure = strTemp1Soure.tolower();
		strTemp2Find = strTemp2Find.tolower();
		const char* pos = strstr(strTemp1Soure, strTemp2Find);
		if (pos != nullptr) {
			return pos - strTemp1Soure;
		}
	}
	return -1;  // NoMatch
}
...
//--------------------------------------------------------------
// GAMEFIX - Added: Function to get file filename from path - chrissstrahl
//--------------------------------------------------------------
...
//--------------------------------------------------------------
// GAMEFIX - Added: Function to put sections into a struct gamefix_iniFileSection taking char* ini-data - generated by ChatGTP 4o - chrissstrahl
//--------------------------------------------------------------
...
//--------------------------------------------------------------
// GAMEFIX - Added: Function to get all section names into a container<str> from a INI file - chrissstrahl
//--------------------------------------------------------------
...
//--------------------------------------------------------------
// GAMEFIX - Added: Function to return contents of given section if it exists - generated by ChatGTP 4o - chrissstrahl
//--------------------------------------------------------------
...
//--------------------------------------------------------------
// GAMEFIX - Added: Function to find the section in which this key resides - first match will be returned - generated by ChatGTP 4o - chrissstrahl
//--------------------------------------------------------------
str gamefix_iniFindSectionByKey(const str& file, const str& file_contents, const str& key)
{
	Container<str> sectionList;
	gamefix_iniSectionsNamesGet(file, sectionList, file_contents);

	for (int i = 1; i <= sectionList.NumObjects(); ++i) {
		str sectionName = sectionList.ObjectAt(i);
		str sectionContent = gamefix_iniSectionGet(file, file_contents, sectionName.c_str());
		str value = gamefix_iniKeyGet(file, sectionContent, key, "");
		if (value.length() > 0) {
			return sectionName;
		}
	}
...
//--------------------------------------------------------------
// GAMEFIX - Added: Function to find the section in which this value resides - first match will be returned - generated by ChatGTP 4o - chrissstrahl
//--------------------------------------------------------------
str gamefix_iniFindSectionByValue(const str& file, const str& file_contents, const str& value) {
	...
	for (int i = 1; i <= sectionList.NumObjects(); ++i) {
		...
		const char* raw = sectionContent.c_str();
		int start = 0;
		int len = sectionContent.length();

		for (int pos = 0; pos <= len; ++pos) {
			if (raw[pos] == '\n' || raw[pos] == '\0') {
				str line = sectionContent.substr(start, pos - start);
				int equalsPos = gamefix_findChar(line.c_str(), '=');

				if (equalsPos != -1 && equalsPos + 1 < line.length()) {
					str val = line.substr(equalsPos + 1, line.length() - equalsPos - 1).tolower();
...
//--------------------------------------------------------------
// GAMEFIX - Added: Function to extract key and value - first match will be returned - generated by ChatGTP 4o - chrissstrahl
//--------------------------------------------------------------
bool gamefix_iniExtractKeyAndValueFromLine(const str& file, const str& line, str& key, str& value)
{
	...
	int commentPos = gamefix_findChar(tempLine.c_str(), ';');
	if (commentPos >= 0) {
		tempLine = gamefix_getStringUntil(tempLine, 0, commentPos);
	}

	// Find '=' separator
	int equalPos = gamefix_findChar(tempLine.c_str(), '=');
	if (equalPos <= 0 || equalPos >= tempLine.length() - 1) {
		return false; // Invalid line format
	}

	// Extract key and value substrings
	key = gamefix_trimWhitespace(gamefix_getStringUntil(tempLine, 0, equalPos),false);
	value = gamefix_trimWhitespace(gamefix_getStringUntil(tempLine, equalPos + 1, tempLine.length()),false);

	// Remove quotes around the value, if any
	if ((value[0] == '"' && value[value.length() - 1] == '"') ||
		(value[0] == '\'' && value[value.length() - 1] == '\''))
	{
		value = gamefix_getStringUntil(value, 1, value.length() - 1);
	}

	// If key is empty, consider this invalid
	if (!key.length()) {
		gi.Printf(va("Key invalid in this line: %s - %s\n", file.c_str(), line.c_str()));
		return false;
	}
...
//--------------------------------------------------------------
// GAMEFIX - Added: Function to return previous and next value from the location of given value  - generated by ChatGTP 4o - chrissstrahl
// 12=value1
// 13=value2 <- given value
// 14=value3
//--------------------------------------------------------------
bool gamefix_iniFindPrevNextValueWrap(
	const str& file,
	const str& file_contents,
	const str& section_name,
	const str& current_value,
	str* out_prev,
	str* out_next
) {
	if (out_prev) *out_prev = "";
	if (out_next) *out_next = "";

	// Sektion-Text holen
	str section = gamefix_iniSectionGet(file, file_contents, section_name);
	if (!section.length())
		return false;

	// Durch numerische Keys 1..N iterieren, bis kein Value mehr gefunden wird.
	// Annahme: Keys sind fortlaufend nummeriert (1,2,3,...).
	str first_value = "";
	str last_value = "";
	str prev_for_current = "";
	str next_for_current = "";
	...
	for (int idx = 1; ; ++idx) {
		...
		// int -> "1","2",... als Key
#if defined(_MSC_VER)
		_snprintf(keybuf, sizeof(keybuf), "%d", idx);
#else
		snprintf(keybuf, sizeof(keybuf), "%d", idx);
#endif
		str key = keybuf;

		// Value lesen (leer = kein weiterer Key)
		str val = gamefix_iniKeyGet(file, section, key, "");
		if (!val.length())
			break;

		// ersten und letzten Wert tracken
		if (!first_value.length())
			first_value = val;

		if (!found_current) {
			// "vorheriger" ist der zuletzt gesehene Wert
			if (val == current_value) {
				found_current = true;
				prev_for_current = last_value; // kann leer sein, wenn current der erste ist
			}
		}
		else {
			// der erste Wert NACH current ist unser "next"
			if (!next_for_current.length())
				next_for_current = val;
			// nicht abbrechen, wir brauchen echtes last_value für "previous" Wrap-Around
		}
	...
	// Wrap-Around anwenden
	if (!next_for_current.length())
		next_for_current = first_value; // am Ende -> springe an den Anfang

	if (!prev_for_current.length())
		prev_for_current = last_value;  // am Anfang -> springe ans Ende

	if (out_prev) *out_prev = prev_for_current;
	if (out_next) *out_next = next_for_current;
	return true;
}

//--------------------------------------------------------------
// GAMEFIX - Added: Function to return previous and next value from the location of given value  - generated by ChatGTP 4o - chrissstrahl
// - WILL CHECK/COMPARE VALUES ONLY UP TO '?' - used for grabbing next and previous map
// 12=value1
// 13=value2 <- given value
// 14=value3
//--------------------------------------------------------------
// Uses gamefix_getStringUntilChar(val, '?', 0) for both the INI values and current_value.
bool gamefix_iniFindPrevNextValueWrap_maplist(
	const str& file,
	const str& file_contents,
	const str& section_name,
	const str& current_value,
	...
	// Compute trimmed (comparison) version of current_value once
	const char DELIM = '?';
	str current_base = gamefix_getStringUntilChar(current_value, DELIM, 0);

	// Iterate numeric keys 1..N
	str first_value = "";
	str last_value = "";
	str prev_for_current = "";
	str next_for_current = "";
	...
	for (int idx = 1; ; ++idx) {
		...
		// Track first/last full values (return values should remain untrimmed)
		if (!first_value.length())
			first_value = val;

		// Compare only up to '?'
		str val_base = gamefix_getStringUntilChar(val, DELIM, 0);

		if (!found_current) {
			if (val_base == current_base) {
				found_current = true;
				prev_for_current = last_value; // may be empty if this is the first
			}
		}
		else {
			if (!next_for_current.length())
				next_for_current = val; // first entry after current
			// keep going to update last_value correctly
		}
	...
	// Wrap-around
	if (!next_for_current.length())
		next_for_current = first_value;

	if (!prev_for_current.length())
		prev_for_current = last_value;
...
str gamefix_replaceForLabelText(str sPure)
{
	...
	for (int i = startIndex; i < sPure.length(); i++) {
		unsigned char ch = (unsigned char)sPure[i];

		// Replace ANSI curly/special quotes with ASCII apostrophe - accent, apostrophe, backtick 
		if (ch == 0xB4 || ch == 0x92 || ch == 0x60)
			sPure[i] = '\'';

		// Newline or hash to tilde
		else if (ch == '\n')
			sPure[i] = '~';
...
//--------------------------------------------------------------
// GAMEFIX - Added: Function to replace Umlaut and other chars that can not be send via sever command - generated by ChatGTP 4o - chrissstrahl
//--------------------------------------------------------------
str gamefix_replaceUmlautAndSpecials(str& text)
{
	...
	for (int i = 0; i < text.length(); i++)
	{
		...
		{
		// Lowercase umlauts
		case 0xF6: result += "oe"; break; // o-uml
		case 0xFC: result += "ue"; break; // u-uml
		case 0xE4: result += "ae"; break; // a-uml

		// Uppercase umlauts
		case 0xD6: result += "Oe"; break; // O-uml
		case 0xDC: result += "Ue"; break; // U-uml
		case 0xC4: result += "Ae"; break; // A-uml

		// Eszett
		case 0xDF: result += "ss"; break; // sharp-s

		// Acute accent, curly apostrophe ? normal apostrophe
		case 0xB4: // accent
